import React, { useState, useRef, useEffect } from 'react';
import { IonIcon , IonToolbar, IonContent, IonPage, IonButtons, IonButton, IonMenuButton, IonSearchbar, IonRefresher, IonRefresherContent, IonModal, IonHeader, getConfig, IonTitle, IonList, IonItem, IonLabel, IonItemSliding, IonAvatar, IonPopover, IonLoading } from '@ionic/react';
import { connect } from '../../../data/connect';
import '../../style.scss'
import * as learnerselectors from './LearnerSelectors';
import { setSearchText } from '../../../data/sessions/sessions.actions';
import { Learner } from './Learner';
import { loadListLearner } from './listLearner.actions';
import { add } from 'ionicons/icons';

interface OwnProps { }

interface StateProps {
  listLearners: Learner[];
  mode: 'ios' | 'md'
}

interface DispatchProps {
  setSearchText: typeof setSearchText;
  loadListLearner: typeof loadListLearner;
}


type ListTeacherPageProps = OwnProps & StateProps & DispatchProps;

const ListTeacherPage: React.FC<ListTeacherPageProps> = ({ listLearners, setSearchText, loadListLearner, mode }) => {
  const [showFilterModal, setShowFilterModal] = useState(false);
  const ionRefresherRef = useRef<HTMLIonRefresherElement>(null);
  const [showCompleteToast, setShowCompleteToast] = useState(false);
  const [showPopover, setShowPopover] = useState(false);

  const doRefresh = () => {
    setTimeout(() => {
      ionRefresherRef.current!.complete();
      setShowCompleteToast(true);
    }, 2500)
  };

  useEffect(() => {
    loadListLearner();
  }, []);

  return (
    <IonPage id="list-user-page" className="list-page">
      <IonHeader>
        <IonToolbar className='custom-toolbar'>
          <IonButtons slot="start">
            <IonMenuButton className='c-white'/>
          </IonButtons>
          <IonTitle className='c-white'>Danh Sách Học Viên</IonTitle>
          <IonButtons slot="end">
            <IonButton routerLink={'/addLearner'}>
              <IonIcon className='c-white' slot="icon-only" icon={add} />
            </IonButton>
          </IonButtons>
        </IonToolbar>

        <IonToolbar className='custom-toolbar'>
          <IonSearchbar
            className="custom-search"
            placeholder="Tìm tên, email, phone"
            onIonChange={(e: any) => setSearchText(e.detail.value)}
          />
        </IonToolbar>
      </IonHeader>

      <IonContent>
        <IonRefresher slot="fixed" ref={ionRefresherRef} onIonRefresh={doRefresh}>
          <IonRefresherContent />
        </IonRefresher>
        <IonLoading message="Please wait..." duration={0} isOpen={(listLearners ? false : true)} />
        <IonList>
          {listLearners && listLearners.map((learner, index: number) => (
            <IonItemSliding key={learner.id}>
              <IonItem routerLink={`/editLearner/${learner.id}`}>
                <IonAvatar slot="start">
                  <img src="/assets/img/person-green-icon.svg"/>
                </IonAvatar>

                <IonLabel>

                  <h3>{learner.user.fullName}</h3>
                  <p>
                    Email: {learner.user.email}
                  </p>
                  <p>
                    Phone: {learner.user.phone}
                  </p>
                </IonLabel>
              </IonItem>
              <IonPopover
                isOpen={showPopover}
                cssClass=''
                onDidDismiss={e => setShowPopover(false)}
              >
                <p>This is popover content</p>
              </IonPopover>
            </IonItemSliding>


          ))}
        </IonList>
      </IonContent>

      <IonModal
        isOpen={showFilterModal}
        onDidDismiss={() => setShowFilterModal(false)}>
      </IonModal>
    </IonPage>
  );
};

export default connect<OwnProps, StateProps, DispatchProps>({
  mapStateToProps: (state) => ({
    listLearners: learnerselectors.getFilteredLearners(state),
    mode: getConfig()!.get('mode')
  }),
  mapDispatchToProps: {
    setSearchText, loadListLearner
  },
  component: React.memo(ListTeacherPage)
});
